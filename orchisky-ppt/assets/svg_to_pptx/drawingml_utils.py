...
EA_FONTS = {
    ...
    'Hiragino Sans', 'Hiragino Sans GB', 'Noto Sans SC', 'Noto Serif SC',
    'Source Han Sans SC', 'Source Han Serif SC', 'Songti SC', 'Songti TC',
}
...
FONT_FALLBACK_WIN = {
    ...
    'Roboto': 'Segoe UI', 'SF Pro': 'Segoe UI', 'SF Pro Display': 'Segoe UI',
    'Menlo': 'Consolas', 'Monaco': 'Consolas', 'SF Mono': 'Consolas',
}
...
_SERIF_LATIN = {
    'Times New Roman', 'Georgia', 'Garamond', 'Cambria', 'SimSun',
...
def ctx_x(val: float, ctx: ConvertContext) -> float:
    return val * ctx.scale_x + getattr(ctx, 'translate_x', 0)


def ctx_y(val: float, ctx: ConvertContext) -> float:
    return val * ctx.scale_y + getattr(ctx, 'translate_y', 0)
...
def resolve_url_id(url_str: str):
    if not url_str:
        return None
...
def parse_font_family(font_family_str: str) -> dict:
    ...
    for font in fonts:
        ...
        if font in GENERIC_FONT_MAP:
            latin_font = latin_font or GENERIC_FONT_MAP[font]
            continue
