...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    ext_cy = px_to_emu(h)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Rectangle {shape_id}', off_x, off_y, ext_cx, ext_cy, geom, fill, stroke, effect, rot=rot),
        bounds_emu=(off_x, off_y, off_x + ext_cx, off_y + ext_cy),
    )


def _build_arc_ring_path(cx, cy, r, stroke_width, dash_len, dash_offset, rotate_deg, sx, sy):
    ...
    angles = [start_angle + (end_angle - start_angle) * i / num_segments for i in range(num_segments + 1)]
    ...
    geom = f'<a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/><a:rect l="l" t="t" r="r" b="b"/><a:pathLst><a:path w="{w_emu}" h="{h_emu}">\n{"".join(lines)}\n</a:path></a:pathLst></a:custGeom>'
    return geom, px_to_emu(min_x), px_to_emu(min_y), w_emu, h_emu


def _is_donut_circle(elem, ctx):
...
def convert_circle(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if _is_donut_circle(elem, ctx):
        ...
        geom, min_x, min_y, w_emu, h_emu = _build_arc_ring_path(
            ctx_x(cx_, ctx) / ctx.scale_x, ctx_y(cy_, ctx) / ctx.scale_y,
            r, stroke_width, dash_len, dash_offset, rotate_deg, ctx.scale_x, ctx.scale_y,
        )
        if not geom:
            return None
        ...
        shape_id = ctx.next_id()
        return ShapeResult(
            xml=_wrap_shape(shape_id, f'Arc {shape_id}', min_x, min_y, w_emu, h_emu, geom, fill, stroke_xml, effect),
    ...
    ext_cy = px_to_emu(h)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Ellipse {shape_id}', off_x, off_y, ext_cx, ext_cy, geom, fill, stroke, effect),
        bounds_emu=(off_x, off_y, off_x + ext_cx, off_y + ext_cy),
    )
...
def convert_line(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    has_marker = bool(_get_attr(elem, 'marker-start', ctx) or _get_attr(elem, 'marker-end', ctx))
    if has_marker:
        w = abs(x2 - x1)
        h = abs(y2 - y1)
        ...
        if flip_h and flip_v:
            ...
            flip_attr = ' flipV="1"'
        rot_attr = f' rot="{rot}"' if rot else ''
        xml = (f'<p:sp><p:nvSpPr><p:cNvPr id="{shape_id}" name="{_xml_escape(f"Line {shape_id}")}"/>'
               f'<p:cNvSpPr/><p:nvPr/></p:nvSpPr><p:spPr>'
               f'<a:xfrm{flip_attr}{rot_attr}><a:off x="{off_x}" y="{off_y}"/>'
               f'<a:ext cx="{w_emu}" cy="{h_emu}"/></a:xfrm>'
               f'<a:prstGeom prst="line"><a:avLst/></a:prstGeom>'
               f'<a:noFill/>{stroke}</p:spPr></p:sp>')
    else:
        w = max(abs(x2 - x1), 1)
        h = max(abs(y2 - y1), 1)
        w_emu = px_to_emu(w)
        ...
        xml = _wrap_shape(shape_id, f'Line {shape_id}', off_x, off_y, w_emu, h_emu, geom, '<a:noFill/>', stroke, rot=rot)
...
def convert_path(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    path_xml, min_x, min_y, width, height = path_commands_to_drawingml(
        commands, ctx.translate_x + tx, ctx.translate_y + ty, ctx.scale_x, ctx.scale_y)
    if not path_xml:
        return None
    w_emu = px_to_emu(width)
    h_emu = px_to_emu(height)
    ...
    off_y = px_to_emu(min_y)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Freeform {shape_id}', off_x, off_y, w_emu, h_emu, geom, fill, stroke, effect, rot=rot),
...
def convert_polygon(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    path_xml, min_x, min_y, width, height = path_commands_to_drawingml(commands, ctx.translate_x, ctx.translate_y, ctx.scale_x, ctx.scale_y)
    if not path_xml:
        return None
    w_emu = px_to_emu(width)
    h_emu = px_to_emu(height)
    ...
    off_y = px_to_emu(min_y)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Polygon {shape_id}', off_x, off_y, w_emu, h_emu, geom, fill, stroke, rot=rot),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
    )
...
def convert_polyline(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    off_y = px_to_emu(min_y)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Polyline {shape_id}', off_x, off_y, w_emu, h_emu, geom, '<a:noFill/>', stroke, rot=rot),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
    )
...
def _build_text_runs(elem, parent_attrs):
    runs = []
    if elem.text:
...
def _has_dy_tspan_linebreaks(elem):
    for child in elem:
        child_tag = child.tag.replace(f'{{{SVG_NS}}}', '')
        if child_tag == 'tspan':
            dy = child.get('dy', '').strip()
            if dy and dy not in ('0', '0em', '0px'):
                return True
    return False


def _parse_em_or_px(value, font_size):
    value = value.strip()
...
def _build_tspan_line_shapes(elem, base_x, base_y, parent_attrs, fonts, algn, text_anchor, effect_xml, ctx):
    results = []
    current_y = base_y
    ...
    for child in children:
        ...
        if child.get('fill'):
            ...
            if c:
                run_attrs['fill'] = c
        if child.get('font-size'):
            run_attrs['font_size'] = _f(child.get('font-size'), font_size)
            font_size = run_attrs['font_size']
        if child.get('font-family'):
            run_attrs['font_family'] = child.get('font-family')
        if child.get('font-style'):
            run_attrs['font_style'] = child.get('font-style')
        tspan_x_raw = child.get('x')
        if tspan_x_raw is not None:
            line_x = ctx_x(_f(tspan_x_raw), ctx)
        else:
            line_x = base_x
        dy_px = _parse_em_or_px(dy_raw, font_size)
        current_y += dy_px
        text = _normalize_text(''.join(child.itertext()))
        if not text:
            continue
        line_width = estimate_text_width(text, font_size, run_attrs.get('font_weight', '400')) * 1.15
        padding = font_size * 0.1
        if text_anchor == 'middle':
            box_x = line_x - line_width / 2 - padding
        elif text_anchor == 'end':
            box_x = line_x - line_width - padding
        else:
            box_x = line_x - padding
        box_y = current_y - font_size * 0.85
        box_w = line_width + padding * 2
        box_h = font_size * 1.5 + padding
        run_xml = _build_run_xml({**run_attrs, 'text': text}, fonts, ctx)
        shape_id = ctx.next_id()
...
<p:spPr><a:xfrm><a:off x="{off_x}" y="{off_y}"/><a:ext cx="{ext_cx}" cy="{ext_cy}"/></a:xfrm>
<a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/><a:ln><a:noFill/></a:ln>{effect_xml}</p:spPr>
<p:txBody><a:bodyPr wrap="none" lIns="0" tIns="0" rIns="0" bIns="0" anchor="t" anchorCtr="0"><a:spAutoFit/></a:bodyPr>
<a:lstStyle/><a:p><a:pPr algn="{algn}"/>{run_xml}</a:p></p:txBody></p:sp>'''
        results.append(ShapeResult(xml=xml, bounds_emu=(off_x, off_y, off_x + ext_cx, off_y + ext_cy)))
    return results


def _build_run_xml(run, default_fonts, ctx=None):
    text = run['text']
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    parent_attrs = {
        'fill': fill_color, 'fill_raw': fill_raw, 'font_weight': font_weight,
        'font_size': font_size, 'font_family': font_family_str, 'font_style': font_style,
        'text_decoration': text_decoration, 'opacity': opacity,
    }
    ...
    if _has_dy_tspan_linebreaks(elem):
        line_shapes = _build_tspan_line_shapes(elem, x, y, parent_attrs, fonts, algn, text_anchor, effect_xml, ctx)
        if not line_shapes:
            return None
        if len(line_shapes) == 1:
            return line_shapes[0]
        combined_xml = '\n'.join(s.xml for s in line_shapes)
        all_bounds = [s.bounds_emu for s in line_shapes if s.bounds_emu]
        if all_bounds:
            min_x = min(b[0] for b in all_bounds)
            min_y = min(b[1] for b in all_bounds)
            max_x = max(b[2] for b in all_bounds)
            max_y = max(b[3] for b in all_bounds)
            return ShapeResult(xml=combined_xml, bounds_emu=(min_x, min_y, max_x, max_y))
        return ShapeResult(xml=combined_xml)
...
<p:nvSpPr><p:cNvPr id="{shape_id}" name="TextBox {shape_id}"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr>
<p:spPr><a:xfrm{rot_attr}><a:off x="{off_x}" y="{off_y}"/><a:ext cx="{ext_cx}" cy="{ext_cy}"/></a:xfrm>
<a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/><a:ln><a:noFill/></a:ln>{effect_xml}</p:spPr>
<p:txBody><a:bodyPr wrap="none" lIns="0" tIns="0" rIns="0" bIns="0" anchor="t" anchorCtr="0"><a:spAutoFit/></a:bodyPr>
<a:lstStyle/><a:p><a:pPr algn="{algn}"/>{runs_xml}</a:p></p:txBody></p:sp>''',
        bounds_emu=(off_x, off_y, off_x + ext_cx, off_y + ext_cy))


def _clip_commands_to_geom(commands, img_x, img_y, img_w, img_h, object_bbox):
    w_emu = px_to_emu(img_w)
    h_emu = px_to_emu(img_h)
    if w_emu <= 0 or h_emu <= 0:
        return '<a:prstGeom prst="rect"><a:avLst/></a:prstGeom>'
    def _tx(x):
        return int(x * w_emu) if object_bbox else px_to_emu(x - img_x)
    def _ty(y):
        return int(y * h_emu) if object_bbox else px_to_emu(y - img_y)
    parts = []
    for cmd in commands:
        if cmd.cmd == 'M':
            parts.append(f'<a:moveTo><a:pt x="{_tx(cmd.args[0])}" y="{_ty(cmd.args[1])}"/></a:moveTo>')
        elif cmd.cmd == 'L':
            parts.append(f'<a:lnTo><a:pt x="{_tx(cmd.args[0])}" y="{_ty(cmd.args[1])}"/></a:lnTo>')
        elif cmd.cmd == 'C':
            pts = ''.join(f'<a:pt x="{_tx(cmd.args[i])}" y="{_ty(cmd.args[i+1])}"/>' for i in range(0, 6, 2))
            parts.append(f'<a:cubicBezTo>{pts}</a:cubicBezTo>')
        elif cmd.cmd == 'Z':
            parts.append('<a:close/>')
    return f'<a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/><a:rect l="l" t="t" r="r" b="b"/><a:pathLst><a:path w="{w_emu}" h="{h_emu}">\n{"".join(parts)}\n</a:path></a:pathLst></a:custGeom>'


def _resolve_clip_geometry(elem, ctx, raw_x, raw_y, raw_w, raw_h):
    ...
    if shape_tag == 'path':
        ...
        if not commands:
            return DEFAULT
        return _clip_commands_to_geom(commands, raw_x, raw_y, raw_w, raw_h, is_obb)
...
def convert_image(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    ctx.rel_entries.append({'id': r_id, 'type': 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image', 'target': f'../media/{img_filename}'})
    rot = 0
    transform = elem.get('transform')
    if transform:
        r_match = re.search(r'rotate\(\s*([-\d.]+)', transform)
...
<p:spPr><a:xfrm{rot_attr}><a:off x="{off_x}" y="{off_y}"/><a:ext cx="{ext_cx}" cy="{ext_cy}"/></a:xfrm>{clip_geom}</p:spPr>
...
def convert_ellipse(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    ext_cy = px_to_emu(h)
    return ShapeResult(
        xml=_wrap_shape(shape_id, f'Ellipse {shape_id}', off_x, off_y, ext_cx, ext_cy, geom, fill, stroke, rot=rot),
        bounds_emu=(off_x, off_y, off_x + ext_cx, off_y + ext_cy),
    )
